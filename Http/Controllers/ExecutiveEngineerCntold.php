           ...
           //dd($Count_Chked_Emb_Stl , $countcombinarray);
            ...
            // dd("Inserted successfilly");
             ...
             $checkeddata = DB::table('embs')
                        ...
                        // dd($DBSectionEngNames);
            return view('ExecutiveEngineerEMB',compact('DBSectionEngNames','max_dye_date','workDetails1','billDate','fund_Hd1','divName1','Recordeno','titemnoRecords','titemno','tbillid','recnovalues','checkeddata'));
            ...
            if (!$measnormaldata->isEmpty() || !$meassteeldata->isEmpty()) {
                ...
                if (in_array(substr($itemid, -6), ["000092", "000093", "001335", "001336", "002016", "002017", "002023", "002024", "003351", "003352", "003878"]))
                {
                    ...
                    if($meassteeldata->isEmpty()){
                        ...
                        <th style="width: 7%; border-color: black;">Breadth</th><th style="width: 7%; border-color: black;">Height</th><th style="width: 7%; border-color: black;">Quantity</th><th style="width: 4%; border-color: black;">Check</th><th style="width: 10%; border-color: black;">Checked Quantity</th>
                        </thead></table>';
                    }
                    ...
                    $html .= '<table style="border-collapse: collapse; width: 100%; border: 1px solid black;"><tbody><td style="border: 1px solid black; padding: 8px; width: 5%;">' . $nordata->sr_no . '</td><td style="border: 1px solid black; padding: 8px; width: 30%; word-wrap: break-word; max-width: 30px;">' . $nordata->parti . '</td>';
                        if($formula)
                        {
                        ...
                        {
                            $html .= '<td style="border: 1px solid black; padding: 35px; width: 4%;"><input id="RselectAll" class="checkboxS form-check-input form-check custom-checkbox" type="checkbox" name="je_check_Item[' .$nordata->meas_id. ']" onclick="CustomeCheckBoxSFun(' . $measdatacount . ');CheckIndicatorinput('.$measidstring.');" checked>
                                    </td>';
                        }
                        else{
                            $html .= '<td style="border: 1px solid black; padding: 35px; width: 4%;"><input id="RselectAll" class="checkboxS form-check-input form-check custom-checkbox" type="checkbox" name="je_check_Item[' .$nordata->meas_id. ']" onclick="CustomeCheckBoxSFun(' . $measdatacount . ');CheckIndicatorinput('.$measidstring.');">';
                        }
                        // dd($dye_chk_date,$BillDt);
                         ...
                         $html .= '<td style="width: 10%; border-color: black;">
...
$EEqty=array_keys($eeqty);
        ...
        elseif($btnall==='all')
        {
            ...
            //Count of combine data...
            $Stldyechkcount1 = DB::table('stlmeas')->where('t_bill_id' , $tbillid)->where('ee_check',"=",1)->get();
            $Stldyechkcount=count($Stldyechkcount1);
            //dd($Stldyechkcount);
          ...
          //  dd($Count_Chked_Emb_Stl , $countcombinarray);
            ...
            ->max('dyE_chk_dt');
            // $percentage=31.28;
            $DyePercentage=$request->percentageTextBox;

            $cleanedString = str_replace('%', '', $DyePercentage);

            // Step 2: Convert the string to a numeric value
            $numericValue = floatval($cleanedString);
            $Amount=$request->AmountTextBox;
            //dd($numericValue,$Amount);
            if($numericValue >= 5){
             //dd("Grater than 5");
                $workid=0;
                DB::table('bills')
            ...
            //    dd($workid,$percentageTextBox,$tbillid);
                ...
                // Output the CSRF token using Laravel's @csrf directive
                ...
                document.addEventListener("DOMContentLoaded", function () {
                    Swal.fire({
                        ...
                        if (result.isConfirmed)
                        {
                            ...
                            // window.location.href = "' . url('noSubmitView/' . $workid . '/' . $tbillid) . '";
                            history.back();
                        }
                    });
                });
    ...
    // Calculate the amount based on the percentage and baseAmount
        ...
        // Select all checkbox......
        return response()->json(['amount'=>$amount ,'formattedPercentage'=>$formattedPercentage]);
    }
